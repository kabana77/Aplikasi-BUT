...
var
  PembayaranARFrm: TPembayaranARFrm;
...
procedure TPembayaranARFrm.Proc_Refresh1;
var
  i           : word;
  vpertama    : boolean;
...
procedure TPembayaranARFrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  PembayaranARFrm:=Nil;
end;

procedure TPembayaranARFrm.FormPaint(Sender: TObject);
begin
...
procedure TPembayaranARFrm.tbRefreshClick(Sender: TObject);
begin
...
procedure TPembayaranARFrm.tbExportClick(Sender: TObject);
begin
   case PageControl1.TabIndex of
   0 :  begin
...
procedure TPembayaranARFrm.TabSheet1Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TPembayaranARFrm.wwDBGrid1IButtonClick(Sender: TObject);
var
  vFixedCol : String;
begin
...
procedure TPembayaranARFrm.dbGrid1UpdateFooter(Sender: TObject);
begin
  qBX.Close;
  qBX.SQL.Clear;
  qBX.SQL.Add('select count(*) as ndata, sum(nilai) as nilai, sum(dibukukan) as dibukukan,sum(adj) as adj, sum(bi_materai) as bi_materai,');
  ...
  dbGrid1.ColumnByName('BI_MATERAI').FooterValue:=FormatFloat('#,#;(#,#);-', qBXBI_MATERAI.AsFloat);
  dbGrid1.ColumnByName('ADJ').FooterValue:=FormatFloat('#,#;(#,#);-', qBXADJ.AsFloat);
  dbGrid1.ColumnByName('SISA').FooterValue:=FormatFloat('#,#;(#,#);-', qBXSISA.AsFloat);
end;

procedure TPembayaranARFrm.FormCreate(Sender: TObject);
begin
  pTop.Color          := clpTop;
  pLeft.Color         := clpLeft;
...
procedure TPembayaranARFrm.tbFilterClick(Sender: TObject);
var
  i : Word;
  vbukti : String;
begin
...
procedure TPembayaranARFrm.dbGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
var
  i : integer;
begin
...
procedure TPembayaranARFrm.dbGrid1CalcCellColors(Sender: TObject; Field: TField;
...
procedure TPembayaranARFrm.pTopClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
...
procedure TPembayaranARFrm.tbPrintClick(Sender: TObject);
var
  vfield_order : String;
begin
  case PageControl1.TabIndex of
...
procedure TPembayaranARFrm.tsInputShow(Sender: TObject);
begin
...
procedure TPembayaranARFrm.lcLokasiEnter(Sender: TObject);
begin
  if not (Sender as TwwDBLookupCombo).LookupTable.Active then
     (Sender as TwwDBLookupCombo).LookupTable.Open;
end;

procedure TPembayaranARFrm.qBMasterBeforeDelete(DataSet: TDataSet);
begin
      ShowMessage('Maaf, anda boleh HAPUS data disii !');
      Abort;
end;

procedure TPembayaranARFrm.qBMasterBeforeEdit(DataSet: TDataSet);
begin
...
procedure TPembayaranARFrm.qBMasterBeforeInsert(DataSet: TDataSet);
begin
      ShowMessage('Maaf, anda tidak berhak TAMBAH data disini!');
      Abort;
end;

procedure TPembayaranARFrm.qBMasterBeforeOpen(DataSet: TDataSet);
begin
  qBMaster.ParamByName('PNO_REG_OS').AsFloat:=vno_reg;
end;

procedure TPembayaranARFrm.Label34Click(Sender: TObject);
begin
  if vCanUnPost then
  begin
    DMFrm.vcatatan:='';
...
procedure TPembayaranARFrm.wwCheckBox1Click(Sender: TObject);
begin
...
procedure TPembayaranARFrm.tsInputDShow(Sender: TObject);
begin
  dbNavigator.DataSource:=dsqBMaster;
end;

procedure TPembayaranARFrm.tbFilter2Click(Sender: TObject);
begin
...
procedure TPembayaranARFrm.qBMasterCARA_BAYARChange(Sender: TField);
begin
...
procedure TPembayaranARFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TPembayaranARFrm.qBMasterKD_PERKChange(Sender: TField);
begin
...
procedure TPembayaranARFrm.qBMasterKD_PERK2Change(Sender: TField);
begin
...
procedure TPembayaranARFrm.wwDBLookupCombo1CloseUp(Sender: TObject;
...
procedure TPembayaranARFrm.qBDetailNewRecord(DataSet: TDataSet);
begin
  qBDetailNO_REG_OS.AsFloat:=qBMasterNO_REG_OS.AsFloat;
end;

procedure TPembayaranARFrm.dbGridDUpdateFooter(Sender: TObject);
begin
  qBDetailX.Close;
  qBDetailX.Open;
  lSisa.Caption:=FormatFloat('0.0,0;(0.0,0);',qBMasterNILAI.AsFloat-qBDetailXVOUCHER_KLAIM.AsFloat-
  ...
  lBayar.Caption:=FormatFloat('0.0,0;(0.0,0);',qBDetailXVOUCHER_KLAIM.AsFloat+
  ...
  dbGridD.ColumnByName('voucher_lain').FooterValue:=FormatFloat('0.0,0;(0.0,0);',qBDetailXvoucher_lain.AsFloat);
  ...
  dbGridD.ColumnByName('sisa_baru').FooterValue:=FormatFloat('0.0,0;(0.0,0);',qBDetailXsisa_baru.AsFloat);
  dbGridD.ColumnByName('bi_materai').FooterValue:=FormatFloat('#,#;(#,#);-',qBDetailXbi_materai.AsFloat);
end;

procedure TPembayaranARFrm.qBDetailXBeforeOpen(DataSet: TDataSet);
begin
  qBDetailX.ParamByName('pno_reg_os').AsFloat:=qBMasterNO_REG_OS.AsFloat;
end;

procedure TPembayaranARFrm.qBDetailBeforeDelete(DataSet: TDataSet);
begin
  if not vCanDel then
  begin
...
procedure TPembayaranARFrm.qBDetailBeforePost(DataSet: TDataSet);
begin
  vispost_new:=qBMasterISPOST2.AsString;
end;

procedure TPembayaranARFrm.qBDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vCanADD then
  begin
...
procedure TPembayaranARFrm.SummaryBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlSisa.Caption:=FormatFloat('#,#;(#,#);-',qBMasterNILAI.AsFloat-qreTotal.Value.dblResult);
end;

procedure TPembayaranARFrm.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo.Caption:=IntToStr(StrToInt(qrlNo.Caption)+1);
end;

procedure TPembayaranARFrm.qrBuktiBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  qrlNo.Caption:='0';
end;

procedure TPembayaranARFrm.tsInputDJurnalShow(Sender: TObject);
var
  vdebet, vkredit : real;
begin
  qJurnal.Close;
...
procedure TPembayaranARFrm.qBDetailNO_REFF_EXTChange(Sender: TField);
begin
  qFaktur.Close;
...
procedure TPembayaranARFrm.BitBtn1Click(Sender: TObject);
begin
...
procedure TPembayaranARFrm.BitBtn2Click(Sender: TObject);
var
  vsisa, vbayar : real;
begin
  if qBMasterISPOST2.AsString='0' then
...
procedure TPembayaranARFrm.dbGridDDblClick(Sender: TObject);
var
  vbayar : real;
begin
  if qBMasterISPOST2.AsString='0' then
...
procedure TPembayaranARFrm.qBMasterBeforePost(DataSet: TDataSet);
begin
  if qBMasterISPOST2.AsString='1' then
  begin
  ...
  begin
      ...
      begin
        if (qBMasterNO_BUKTI2.AsString='') or (FormatDateTime('YYMM',qBMasterTGL.AsDateTime)<>copy(qBMasterNO_BUKTI2.AsString,5,4)) then
        begin
          DMFrm.Fno_Bukti.Close;
          DMFrm.Fno_Bukti.ParamByName('pkode_form').AsString:='120';
          DMFrm.Fno_Bukti.ParamByName('ptgl').AsDateTime:=qBMasterTGL.AsDateTime;
          DMFrm.Fno_Bukti.Open;
          qBMasterNO_BUKTI2.AsString:=DMFrm.Fno_BuktiNO_BUKTI.AsString;
        end;
      end;
  end;
...
procedure TPembayaranARFrm.qB1BeforeOpen(DataSet: TDataSet);
begin
...
procedure TPembayaranARFrm.qBXBeforeOpen(DataSet: TDataSet);
begin
  case rgStatus.ItemIndex of
...
procedure TPembayaranARFrm.CheckBox1Click(Sender: TObject);
begin
