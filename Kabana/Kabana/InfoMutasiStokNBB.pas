...
type
  TInfoMutasiStokNBBFrm = class(TForm)
    pLeft: TPanel;
    pCenter: TPanel;
    pRight: TPanel;
    pTop: TPanel;
...
var
  InfoMutasiStokNBBFrm: TInfoMutasiStokNBBFrm;

implementation

uses dm, Filter, InfoMutasiStok, InfoMutasiStokRev, InfoMutasiStokBB;

{$R *.dfm}

procedure TInfoMutasiStokNBBFrm.Proc_Refresh1;
var
  i           : word;
  vpertama    : boolean;
...
procedure TInfoMutasiStokNBBFrm.Proc_Refresh2;
var
  i           : word;
  vpertama    : boolean;
...
procedure TInfoMutasiStokNBBFrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  InfoMutasiStokNBBFrm:=Nil;
end;

procedure TInfoMutasiStokNBBFrm.FormPaint(Sender: TObject);
begin
...
procedure TInfoMutasiStokNBBFrm.tbRefreshClick(Sender: TObject);
begin
  ...
  0 : begin
        ...
        qB1.SQL.Add('where nama_prinsipal like :pnama_prinsipal and kelompok in (''PERALATAN PRODUKSI'', ''PERALATAN KANTOR'', ''KOMUTER & ASSESORIS'', ''RUMAH TANGGA & UMUM'', ''LAIN-LAIN'') )');
...
procedure TInfoMutasiStokNBBFrm.tbExportClick(Sender: TObject);
begin
   case PageControl1.TabIndex of
   0 :  begin
...
procedure TInfoMutasiStokNBBFrm.TabSheet1Show(Sender: TObject);
var
  i : integer;
...
procedure TInfoMutasiStokNBBFrm.wwDBGrid1IButtonClick(Sender: TObject);
var
  vFixedCol : String;
begin
...
procedure TInfoMutasiStokNBBFrm.dbGrid1UpdateFooter(Sender: TObject);
begin
  {
  if vsql_org1<>'' then
  begin
...
procedure TInfoMutasiStokNBBFrm.FormCreate(Sender: TObject);
begin
  pTop.Color          := clpTop;
  pLeft.Color         := clpLeft;
...
procedure TInfoMutasiStokNBBFrm.tbFilterClick(Sender: TObject);
var
  i : Word;
begin
...
procedure TInfoMutasiStokNBBFrm.dbGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TInfoMutasiStokNBBFrm.dbGrid1CalcCellColors(Sender: TObject; Field: TField;
...
procedure TInfoMutasiStokNBBFrm.pTopClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
...
procedure TInfoMutasiStokNBBFrm.tbPrintClick(Sender: TObject);
begin
  case PageControl1.TabIndex of
  0 : begin
...
procedure TInfoMutasiStokNBBFrm.qB1BeforeOpen(DataSet: TDataSet);
begin
  qB1.ParamByName('pnama_prinsipal').AsString:='%'
end;

procedure TInfoMutasiStokNBBFrm.CheckBox1Click(Sender: TObject);
begin
  lcPrinsipal.Enabled:=not CheckBox1.Checked;
end;

procedure TInfoMutasiStokNBBFrm.qBXBeforeOpen(DataSet: TDataSet);
begin
...
procedure TInfoMutasiStokNBBFrm.qB2BeforeOpen(DataSet: TDataSet);
begin
  qB2.ParamByName('pkd_item').AsString:=qB1KD_ITEM.AsString;
  qB2.ParamByName('pkd_lokasi').AsString:='5';
end;

procedure TInfoMutasiStokNBBFrm.tbFilter2Click(Sender: TObject);
begin
...
procedure TInfoMutasiStokNBBFrm.TabSheet2Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TInfoMutasiStokNBBFrm.qB1FilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
  {Accept:=(
...
procedure TInfoMutasiStokNBBFrm.CheckBox3Click(Sender: TObject);
begin
  qB1.Filtered:=CheckBox3.Checked;
end;

procedure TInfoMutasiStokNBBFrm.TabSheet3Show(Sender: TObject);
begin
  qB5.Close;
...
procedure TInfoMutasiStokNBBFrm.qB1CalcFields(DataSet: TDataSet);
begin
