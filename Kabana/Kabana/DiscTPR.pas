...
type
  TDiscTPRFrm = class(TForm)
    ...
    qBMasterDASAR_DISC: TStringField;
    qBDetail3DASAR_DISC: TStringField;
    wwDBComboBox4: TwwDBComboBox;
    Label13: TLabel;
    qBMasterKELOMPOK: TStringField;
    qB1KELOMPOK: TStringField;
    qBDetail3KOLOM: TFloatField;
    cbMix2: TwwDBComboBox;
    qBDetailDISC1: TFloatField;
    qBDetailDISC2: TFloatField;
    qBDetailDISC3: TFloatField;
    qBDetailDISC4: TFloatField;
    qBDetailDISC5: TFloatField;
    ...
    procedure qBMasterJENISChange(Sender: TField);
  private
...
procedure TDiscTPRFrm.Proc_Refresh1;
var
  i           : word;
  vpertama    : boolean;
...
procedure TDiscTPRFrm.Proc_Refresh2;
var
  i           : word;
  vpertama    : boolean;
  vFieldName  : array[0..50] of String;
...
procedure TDiscTPRFrm.Proc_Refresh3;
var
  i           : word;
  vpertama    : boolean;
  vFieldName  : array[0..50] of String;
...
procedure TDiscTPRFrm.Proc_Refresh5;
var
  i           : word;
  vpertama    : boolean;
  vFieldName  : array[0..50] of String;
...
procedure TDiscTPRFrm.Proc_Refresh6;
var
  i           : word;
  vpertama    : boolean;
  vFieldName  : array[0..50] of String;
...
procedure TDiscTPRFrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  DiscTPRFrm:=Nil;
end;

procedure TDiscTPRFrm.FormPaint(Sender: TObject);
begin
...
procedure TDiscTPRFrm.tbRefreshClick(Sender: TObject);
begin
...
procedure TDiscTPRFrm.tbExportClick(Sender: TObject);
begin
   case PageControl1.TabIndex of
   0 :  begin
...
procedure TDiscTPRFrm.TabSheet1Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TDiscTPRFrm.wwDBGrid1IButtonClick(Sender: TObject);
var
  vFixedCol : String;
begin
...
procedure TDiscTPRFrm.dbGrid1UpdateFooter(Sender: TObject);
begin
  qBX.Close;
  qBX.SQL.Clear;
...
procedure TDiscTPRFrm.FormCreate(Sender: TObject);
begin
  pTop.Color          := clpTop;
  pLeft.Color         := clpLeft;
...
procedure TDiscTPRFrm.tbFilterClick(Sender: TObject);
var
  i : Word;
  vbukti : String;
begin
...
procedure TDiscTPRFrm.dbGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
var
  i : integer;
begin
...
procedure TDiscTPRFrm.dbGrid1CalcCellColors(Sender: TObject; Field: TField;
...
procedure TDiscTPRFrm.pTopClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
...
procedure TDiscTPRFrm.tbPrintClick(Sender: TObject);
var
  vfield_order : String;
begin
  case PageControl1.TabIndex of
...
procedure TDiscTPRFrm.tsInputShow(Sender: TObject);
begin
  ...
  qBDetail3.Close;
  qBDetail3.Open;
  dbGridD3.ColumnByName('DISC_PSN').ReadOnly:=not (qBMasterJENIS.AsString='TPR UANG %');
  dbGridD3.ColumnByName('DISC_RP').ReadOnly:=not (qBMasterJENIS.AsString='TPR UANG RP');
  dbGridD3.ColumnByName('KD_ITEM').ReadOnly:=not (qBMasterJENIS.AsString='TPR BARANG');
  dbGridD3.ColumnByName('QTY_A').ReadOnly:=not (qBMasterJENIS.AsString='TPR BARANG');
  dbGridD3.ColumnByName('QTY_T').ReadOnly:=not (qBMasterJENIS.AsString='TPR BARANG');
  dbGridD3.ColumnByName('QTY_D').ReadOnly:=not (qBMasterJENIS.AsString='TPR BARANG');
end;

procedure TDiscTPRFrm.qBMasterNewRecord(DataSet: TDataSet);
begin
  qBMasterKD_FORM.AsString:=vkd_transaksi;
  qBMasterTGL.AsDateTime:=vtgl_bukti;
  qBMasterDASAR_RING.AsString:='JUMLAH (ATAS)';
  qBMasterDASAR_DISC.AsString:='HARGA SATUAN';
  qBMasterJENIS.AsString:='TPR UANG %';
  qBMasterKELOMPOK.AsString:='TPR';
end;

procedure TDiscTPRFrm.qBMasterBeforeDelete(DataSet: TDataSet);
begin
  if not vCanDel then
  begin
...
procedure TDiscTPRFrm.qBMasterBeforeEdit(DataSet: TDataSet);
begin
...
procedure TDiscTPRFrm.qBMasterBeforeInsert(DataSet: TDataSet);
begin
  if not vCanADD then
  begin
...
procedure TDiscTPRFrm.qBMasterBeforeOpen(DataSet: TDataSet);
begin
  qBMaster.ParamByName('PNO_REG_OS').AsFloat:=vno_reg;
end;

procedure TDiscTPRFrm.qBDetailNewRecord(DataSet: TDataSet);
begin
  qBDetailNO_REG_OS.AsFloat:=qBMasterNO_REG_OS.AsFloat;
end;

procedure TDiscTPRFrm.qBMasterBeforePost(DataSet: TDataSet);
begin
  vno_reg:=qBMasterNO_REG_OS.AsFloat;
  qBMasterMODE_INPUT.AsString:='GUI';
  vispost_new:=qBMasterISPOST.AsString;
  if ((qBMasterDASAR_DISC.AsString='-') and (qBMasterJENIS.AsString<>'TPR BARANG')) then
  begin
          ShowMessage('Dasar Potongan atau Jenis Discount tidak tepat !');
          Abort;
  end;
...
procedure TDiscTPRFrm.qBDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vCanADD then
  begin
      ShowMessage('Maaf, anda tidak berhak TAMBAH data !');
...
procedure TDiscTPRFrm.lcdItemEnter(Sender: TObject);
begin
  qItem.Open;
end;

procedure TDiscTPRFrm.lcdItemUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  qItem.Close;
  qItem.Open;
...
procedure TDiscTPRFrm.qBDetailKD_ITEMChange(Sender: TField);
begin
...
procedure TDiscTPRFrm.dbGridDUpdateFooter(Sender: TObject);
begin
  qBDetailX.Close;
  qBDetailX.Open;
...
procedure TDiscTPRFrm.qBDetailXBeforeOpen(DataSet: TDataSet);
begin
  qBDetailX.ParamByName('pno_reg_os').AsFloat:=qBMasterNO_REG_OS.AsFloat;
end;

procedure TDiscTPRFrm.qBDetailBeforePost(DataSet: TDataSet);
begin
  vModeInput:=True;
end;

procedure TDiscTPRFrm.Label34Click(Sender: TObject);
begin
  if vCanUnPost then
  begin
     vno_reg:=qBMasterNO_REG_OS.AsFloat;
...
procedure TDiscTPRFrm.wwCheckBox1Click(Sender: TObject);
begin
...
procedure TDiscTPRFrm.qBDetailAfterScroll(DataSet: TDataSet);
begin
...
procedure TDiscTPRFrm.dbGridDEnter(Sender: TObject);
begin
  if qBMaster.State<>dsBrowse then
    qBMaster.Post;
end;

procedure TDiscTPRFrm.tsInputDShow(Sender: TObject);
begin
  dbNavigator.DataSource:=dsqBMaster;
end;

procedure TDiscTPRFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TDiscTPRFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TDiscTPRFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo.Caption:='0';
end;

procedure TDiscTPRFrm.qrBuktiBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vhal:=0;
end;

procedure TDiscTPRFrm.PageHeaderBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  vhal:=vhal+1;
end;

procedure TDiscTPRFrm.TabSheet2Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TDiscTPRFrm.dbGrid2UpdateFooter(Sender: TObject);
begin
  qB2X.Close;
  qB2X.SQL.Clear;
...
procedure TDiscTPRFrm.dbGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
var
  i : integer;
begin
...
procedure TDiscTPRFrm.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo2D.Caption:='0';
...
procedure TDiscTPRFrm.qrMaster2BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  qrlNo2.Caption:='0';
end;

procedure TDiscTPRFrm.QRBand4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo2D.Caption:=IntToStr(StrToInt(qrlNo2D.Caption)+1);
end;

procedure TDiscTPRFrm.qB3CalcFields(DataSet: TDataSet);
var
  vjumlah : String;
begin
  vjumlah:='';
...
procedure TDiscTPRFrm.qB5CalcFields(DataSet: TDataSet);
var
  vjumlah : String;
begin
  vjumlah:='';
...
procedure TDiscTPRFrm.qB6CalcFields(DataSet: TDataSet);
var
  vjumlah : String;
begin
  vjumlah:='';
...
procedure TDiscTPRFrm.dbGrid5TitleButtonClick(Sender: TObject;
  AFieldName: String);
var
  i : integer;
begin
...
procedure TDiscTPRFrm.dbGrid6TitleButtonClick(Sender: TObject;
  AFieldName: String);
var
  i : integer;
begin
...
procedure TDiscTPRFrm.dbGrid3UpdateFooter(Sender: TObject);
begin
  qB2X.Close;
  qB2X.SQL.Clear;
  qB2X.SQL.Add('select count(*) as ndata from (');
...
procedure TDiscTPRFrm.dbGrid5UpdateFooter(Sender: TObject);
begin
  qB2X.Close;
  qB2X.SQL.Clear;
  qB2X.SQL.Add('select count(*) as ndata from (');
...
procedure TDiscTPRFrm.dbGrid6UpdateFooter(Sender: TObject);
begin
  qB2X.Close;
  qB2X.SQL.Clear;
  qB2X.SQL.Add('select count(*) as ndata from (');
...
procedure TDiscTPRFrm.TabSheet3Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TDiscTPRFrm.TabSheet5Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TDiscTPRFrm.TabSheet6Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TDiscTPRFrm.qrMaster5BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  qrlNo5.Caption:='0';
end;

procedure TDiscTPRFrm.QRBand16BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TDiscTPRFrm.QRGroup5BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo5D.Caption:='0';
...
procedure TDiscTPRFrm.QRBand18BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo5D.Caption:=IntToStr(StrToInt(qrlNo5D.Caption)+1);
end;

procedure TDiscTPRFrm.qrMaster6BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  qrlNo6.Caption:='0';
end;

procedure TDiscTPRFrm.QRBand23BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TDiscTPRFrm.QRBand25BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo6D.Caption:=IntToStr(StrToInt(qrlNo6D.Caption)+1);
end;

procedure TDiscTPRFrm.QRGroup2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo6D.Caption:='0';
...
procedure TDiscTPRFrm.qrMaster3BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  qrlNo3.Caption:='0';
end;

procedure TDiscTPRFrm.QRBand32BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlNo3.Caption:=IntToStr(StrToInt(qrlNo3.Caption)+1);
end;

procedure TDiscTPRFrm.QRBand30BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TDiscTPRFrm.tbFilter2Click(Sender: TObject);
begin
...
procedure TDiscTPRFrm.Button1Click(Sender: TObject);
begin
  ProcADDItem.Close;
...
procedure TDiscTPRFrm.lcdOutletUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  qOutlet.Close;
  qOutlet.Open;
...
procedure TDiscTPRFrm.lcdOutletEnter(Sender: TObject);
begin
  qOutlet.Open;
end;

procedure TDiscTPRFrm.qBDetail2ID_OUTLETChange(Sender: TField);
begin
...
procedure TDiscTPRFrm.tsInputD2Show(Sender: TObject);
begin
  qBDetail2.Open;
end;

procedure TDiscTPRFrm.qBDetail2XBeforeOpen(DataSet: TDataSet);
begin
  qBDetail2X.ParamByName('pno_reg_os').AsFloat:=qBMasterNO_REG_OS.AsFloat;
end;

procedure TDiscTPRFrm.dbGridD2UpdateFooter(Sender: TObject);
begin
  qBDetail2X.Close;
  qBDetail2X.Open;
...
procedure TDiscTPRFrm.Button2Click(Sender: TObject);
begin
  ProcADDOutlet.Close;
...
procedure TDiscTPRFrm.qBDetail3NewRecord(DataSet: TDataSet);
begin
  qBDetail3DASAR_RING.AsString:=qBMasterDASAR_RING.AsString;
  qBDetail3DASAR_DISC.AsString:=qBMasterDASAR_DISC.AsString;
  qBDetail3NO_REG_OS.AsFloat:=qBMasterNO_REG_OS.AsFloat;
end;

procedure TDiscTPRFrm.tsInputD3Show(Sender: TObject);
begin
  qBDetail3.Close;
  dbGridD3.ColumnByName('DISC_PSN').ReadOnly:=not (qBMasterJENIS.AsString='TPR UANG %');
  dbGridD3.ColumnByName('DISC_RP').ReadOnly:=not (qBMasterJENIS.AsString='TPR UANG RP');
  dbGridD3.ColumnByName('KOLOM').ReadOnly:=not ((qBMasterKELOMPOK.AsString='TPR MIX2') or (qBMasterKELOMPOK.AsString='REG MIX2'));
  dbGridD3.ColumnByName('KD_ITEM').ReadOnly:=not (qBMasterJENIS.AsString='TPR BARANG');
  dbGridD3.ColumnByName('QTY_A').ReadOnly:=not (qBMasterJENIS.AsString='TPR BARANG');
  dbGridD3.ColumnByName('QTY_T').ReadOnly:=not (qBMasterJENIS.AsString='TPR BARANG');
  dbGridD3.ColumnByName('QTY_D').ReadOnly:=not (qBMasterJENIS.AsString='TPR BARANG');
...
procedure TDiscTPRFrm.qBMasterAfterPost(DataSet: TDataSet);
begin
  PageControl2.ActivePageIndex:=0;
end;

procedure TDiscTPRFrm.qBDetail3KD_ITEMChange(Sender: TField);
begin
...
procedure TDiscTPRFrm.qBMasterJENISChange(Sender: TField);
begin
  if qBMasterJENIS.AsString='TPR BARANG' then
    qBMasterDASAR_DISC.AsString:='-';
end;
