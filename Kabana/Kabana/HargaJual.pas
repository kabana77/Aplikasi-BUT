...
type
  THargaJualFrm = class(TForm)
    ...
    qB1PROFIT_PSN: TFloatField;
    qB1PROFIT_RP: TFloatField;
    qB2TIPE_HARGA: TStringField;
    qB2PROFIT: TFloatField;
    lookTipeHrgJual: TwwDBLookupCombo;
    ...
    procedure lookTipeHrgJualEnter(Sender: TObject);
    procedure qB2AfterPost(DataSet: TDataSet);
    procedure qB1AfterInsert(DataSet: TDataSet);
  private
...
procedure THargaJualFrm.Proc_Refresh1;
var
  i           : word;
  vpertama    : boolean;
...
procedure THargaJualFrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  HargaJualFrm:=Nil;
end;

procedure THargaJualFrm.FormPaint(Sender: TObject);
begin
...
procedure THargaJualFrm.qB1BeforeInsert(DataSet: TDataSet);
begin
  if not vCanADD then
  begin
...
procedure THargaJualFrm.qB1BeforeEdit(DataSet: TDataSet);
begin
...
procedure THargaJualFrm.qB1BeforeDelete(DataSet: TDataSet);
begin
  if not vCanDel then
  begin
...
procedure THargaJualFrm.tbRefreshClick(Sender: TObject);
begin
  qB1.Refresh;
end;

procedure THargaJualFrm.tbExportClick(Sender: TObject);
begin
...
procedure THargaJualFrm.TabSheet1Show(Sender: TObject);
var
  i : integer;
begin
...
procedure THargaJualFrm.wwDBGrid1IButtonClick(Sender: TObject);
var
  vFixedCol : String;
begin
...
procedure THargaJualFrm.dbGrid1UpdateFooter(Sender: TObject);
begin
  if vsql_org1<>'' then
  begin
    qBX.Close;
...
procedure THargaJualFrm.FormCreate(Sender: TObject);
begin
  pTop.Color          := clpTop;
  pLeft.Color         := clpLeft;
...
procedure THargaJualFrm.tbFilterClick(Sender: TObject);
var
  i : Word;
begin
...
procedure THargaJualFrm.dbGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure THargaJualFrm.dbGrid1CalcCellColors(Sender: TObject; Field: TField;
...
procedure THargaJualFrm.pTopClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
...
procedure THargaJualFrm.tbPrintClick(Sender: TObject);
begin
...
procedure THargaJualFrm.lcdItemEnter(Sender: TObject);
begin
  qItem.Open;
end;

procedure THargaJualFrm.lcdItemUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  qItem.Close;
  qItem.Open;
...
procedure THargaJualFrm.lcdItemCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
     qB1NAMA_PRINSIPAL.AsString:=qItemNAMA_PRINSIPAL.AsString;
     qB1NAMA_ITEM.AsString:=qItemNAMA_ITEM.AsString;
     qB1BRAND.AsString:=qItemBRAND.AsString;
     qB1KELOMPOK.AsString:=qItemKELOMPOK.AsString;
     qB1RA.AsFloat:=qItemRA.AsFloat;
     qB1RT.AsFloat:=qItemRT.AsFloat;
     qB1RD.AsFloat:=qItemRD.AsFloat;
     qB1SAT_A.AsFloat:=qItemSAT_A.AsFloat;
     qB1SAT_T.AsFloat:=qItemSAT_T.AsFloat;
     qB1SAT_D.AsFloat:=qItemSAT_D.AsFloat;
     qB1BKP.AsString:=qItemBKP.AsString;
     qB1PPN.AsString:=qItemPPN.AsString;
  end;
end;

procedure THargaJualFrm.BitBtn1Click(Sender: TObject);
begin
      ShowMessage('Maaf sementara Feature ini belum diberlakukan !');
end;

procedure THargaJualFrm.TabSheet2Show(Sender: TObject);
begin
  qB2.Open;
end;

procedure THargaJualFrm.lookTipeHrgJualEnter(Sender: TObject);
begin
  DMFrm.qTipeHrgJual.Open;
end;

procedure THargaJualFrm.qB2AfterPost(DataSet: TDataSet);
begin
  DMFrm.qTipeHrgJual.Close;
  DMFrm.qTipeHrgJual.Open;
end;

procedure THargaJualFrm.qB1AfterInsert(DataSet: TDataSet);
begin
  DMFrm.qseq.close;
  DMFrm.qseq.open;
  qB1NO_REG.AsFloat:=DMFrm.qSeqSEQ.AsFloat;
end;
