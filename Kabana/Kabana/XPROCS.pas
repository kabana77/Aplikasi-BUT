...
function  strEmpty(const S: String): Boolean;   { 1.2.96 }
...
begin
  ...
  for i := 0 to length(S) do
    if S[i] in DIGITS then
      a[j] := a[j]+S[i]
    else if S[i] in [',','.',':',DateSeparator] then Inc(j);  { Changed KM 17.4.97 }
  for i := 0 to 2 do
...
var
  SearchRec       : TSearchRec;
begin
  if FindFirst(aFile,faAnyFile,SearchRec)=0
    then Result:=FileDateToDateTime(SearchRec.Time)
    else Result:=0;
  FindClose(SearchRec);
end;
