...
type
  TInfoMutasiStokFrm = class(TForm)
    pLeft: TPanel;
    pCenter: TPanel;
    pRight: TPanel;
    pTop: TPanel;
...
var
  InfoMutasiStokFrm: TInfoMutasiStokFrm;
...
procedure TInfoMutasiStokFrm.Proc_Refresh1;
var
  i           : word;
  vpertama    : boolean;
...
procedure TInfoMutasiStokFrm.Proc_Refresh2;
var
  i           : word;
  vpertama    : boolean;
...
procedure TInfoMutasiStokFrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  InfoMutasiStokFrm:=Nil;
end;

procedure TInfoMutasiStokFrm.FormPaint(Sender: TObject);
begin
...
procedure TInfoMutasiStokFrm.tbRefreshClick(Sender: TObject);
begin
...
procedure TInfoMutasiStokFrm.tbExportClick(Sender: TObject);
begin
   case PageControl1.TabIndex of
   0 :  begin
...
procedure TInfoMutasiStokFrm.TabSheet1Show(Sender: TObject);
var
  i : integer;
begin
...
procedure TInfoMutasiStokFrm.wwDBGrid1IButtonClick(Sender: TObject);
var
  vFixedCol : String;
begin
...
procedure TInfoMutasiStokFrm.dbGrid1UpdateFooter(Sender: TObject);
begin
  if vsql_org1<>'' then
  begin
    qBX.Close;
    ...
    dbGrid1.ColumnByName('nilai_awal').FooterValue:=FormatFloat('#,#;(#,#);-', qBXnilai_awal.AsFloat);
    ...
    dbGrid1.ColumnByName('nilai_akhir').FooterValue:=FormatFloat('#,#;(#,#);-', qBXnilai_akhir.AsFloat);
  end
  else
    pLeft2.Caption:='';
end;

procedure TInfoMutasiStokFrm.FormCreate(Sender: TObject);
begin
  pTop.Color          := clpTop;
  pLeft.Color         := clpLeft;
...
procedure TInfoMutasiStokFrm.tbFilterClick(Sender: TObject);
var
  i : Word;
begin
...
procedure TInfoMutasiStokFrm.dbGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TInfoMutasiStokFrm.dbGrid1CalcCellColors(Sender: TObject; Field: TField;
...
procedure TInfoMutasiStokFrm.pTopClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
...
procedure TInfoMutasiStokFrm.tbPrintClick(Sender: TObject);
begin
  case PageControl1.TabIndex of
  0 : begin
...
procedure TInfoMutasiStokFrm.qB1BeforeOpen(DataSet: TDataSet);
begin
...
procedure TInfoMutasiStokFrm.CheckBox1Click(Sender: TObject);
begin
  lcPrinsipal.Enabled:=not CheckBox1.Checked;
end;

procedure TInfoMutasiStokFrm.qBXBeforeOpen(DataSet: TDataSet);
begin
  if CheckBox1.Checked then
...
procedure TInfoMutasiStokFrm.qB2BeforeOpen(DataSet: TDataSet);
begin
  qB2.ParamByName('pkd_item').AsString:=qB1KD_ITEM.AsString;
  qB2.ParamByName('pkd_lokasi').AsString:=lcLokasi.Text;
end;

procedure TInfoMutasiStokFrm.tbFilter2Click(Sender: TObject);
begin
...
procedure TInfoMutasiStokFrm.TabSheet2Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TInfoMutasiStokFrm.qB1FilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
  Accept:=(qB1QTY_X.AsFloat<>0);
end;

procedure TInfoMutasiStokFrm.CheckBox3Click(Sender: TObject);
begin
  qB1.Filtered:=CheckBox3.Checked;
end;

procedure TInfoMutasiStokFrm.TabSheet3Show(Sender: TObject);
begin
  qB5.Close;
