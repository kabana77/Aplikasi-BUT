...
uses
  ...
  QuickRpt, Wwdbdlg, Mask, wwdbedit, Wwdotdot, Wwdbcomb;

type
  TDaftarItemImporFrm = class(TForm)
    pLeft: TPanel;
    pCenter: TPanel;
    pRight: TPanel;
    pTop: TPanel;
    ...
    qB1NO_PIB: TFloatField;
    qB1NO_PEB: TFloatField;
    qB1JML: TFloatField;
    qB1HRG: TFloatField;
    qB1DISC: TFloatField;
    qB1MU: TStringField;
    ...
    qB1PPH: TFloatField;
    qB1PPNBN: TFloatField;
    qB1KG_BERSIH: TFloatField;
    qB1JML_ISI: TFloatField;
    qB1PPN_BBS: TFloatField;
    qB1PPH_BBS: TFloatField;
    qB1PPNBN_BBS: TFloatField;
    qB1ISIMPOR: TStringField;
    qB1COLOR: TStringField;
    qB1KD_PARENT: TStringField;
...
var
  DaftarItemImporFrm: TDaftarItemImporFrm;
...
procedure TDaftarItemImporFrm.Proc_Refresh1;
var
  i           : word;
  vpertama    : boolean;
...
procedure TDaftarItemImporFrm.Proc_Refresh2;
var
  i           : word;
  vpertama    : boolean;
  vFieldName  : array[0..50] of String;
...
procedure TDaftarItemImporFrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  DaftarItemImporFrm:=Nil;
end;

procedure TDaftarItemImporFrm.FormPaint(Sender: TObject);
begin
...
procedure TDaftarItemImporFrm.qB1BeforeInsert(DataSet: TDataSet);
begin
  if not vCanADD then
  begin
...
procedure TDaftarItemImporFrm.qB1BeforeEdit(DataSet: TDataSet);
begin
...
procedure TDaftarItemImporFrm.qB1BeforeDelete(DataSet: TDataSet);
begin
  if not vCanDel then
  begin
...
procedure TDaftarItemImporFrm.tbRefreshClick(Sender: TObject);
begin
...
procedure TDaftarItemImporFrm.tbExportClick(Sender: TObject);
begin
   case PageControl1.TabIndex of
   0 :  begin
...
procedure TDaftarItemImporFrm.TabSheet1Show(Sender: TObject);
var
  i : integer;
begin
...
procedure TDaftarItemImporFrm.wwDBGrid1IButtonClick(Sender: TObject);
var
  vFixedCol : String;
begin
...
procedure TDaftarItemImporFrm.dbGrid1UpdateFooter(Sender: TObject);
begin
  if vsql_org1<>'' then
  begin
    qBX.Close;
...
procedure TDaftarItemImporFrm.FormCreate(Sender: TObject);
begin
  pTop.Color          := clpTop;
  pLeft.Color         := clpLeft;
...
procedure TDaftarItemImporFrm.tbFilterClick(Sender: TObject);
var
  i : Word;
begin
...
procedure TDaftarItemImporFrm.dbGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TDaftarItemImporFrm.dbGrid1CalcCellColors(Sender: TObject; Field: TField;
...
procedure TDaftarItemImporFrm.pTopClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
...
procedure TDaftarItemImporFrm.tbPrintClick(Sender: TObject);
begin
...
procedure TDaftarItemImporFrm.lcPrinsipalEnter(Sender: TObject);
begin
  if not (Sender as TwwDBLookupCombo).LookupTable.Active then
     (Sender as TwwDBLookupCombo).LookupTable.Open;
end;

procedure TDaftarItemImporFrm.lcdKelompokEnter(Sender: TObject);
begin
...
procedure TDaftarItemImporFrm.lcdKelompokUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
var
  cfilter : String;
begin
...
procedure TDaftarItemImporFrm.qB1NewRecord(DataSet: TDataSet);
begin
  qB1PPN.AsString:='EXC';
  qB1ISIMPOR.AsString:='1';
  qB1NO_PIB.AsString:='19900000';
end;

procedure TDaftarItemImporFrm.lcdKelompokUserButton2Click(Sender: TObject;
  LookupTable: TDataSet);
begin
    sqDaftarItemKelompokLook.Close;
    sqDaftarItemKelompokLook.ParamByName('pbrand').AsString:='%';
...
procedure TDaftarItemImporFrm.TabSheet2Show(Sender: TObject);
var
  i : integer;
begin
...
procedure TDaftarItemImporFrm.dbGrid2UpdateFooter(Sender: TObject);
begin
  if vsql_org2<>'' then
  begin
    qBX.Close;
...
procedure TDaftarItemImporFrm.dbGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TDaftarItemImporFrm.qB2NewRecord(DataSet: TDataSet);
begin
  qB2ID_PRINSIPAL.AsString:='01';
end;

procedure TDaftarItemImporFrm.qB1BeforePost(DataSet: TDataSet);
begin
