...
type
  TInfoKartuPiutangFrm = class(TForm)
    pLeft: TPanel;
    pCenter: TPanel;
    pRight: TPanel;
    pTop: TPanel;
    ...
    qB1TGL_LUNAS: TDateTimeField;
    qB1BUKTI_LUNAS: TFloatField;
    qB1MU: TStringField;
    TabSheet4: TTabSheet;
    dbGrid4: TwwDBGrid;
...
var
  InfoKartuPiutangFrm: TInfoKartuPiutangFrm;

implementation

uses dm, Filter, KartuPiutang, DateUtils;

{$R *.dfm}

procedure TInfoKartuPiutangFrm.Proc_Refresh1;
var
  i           : word;
  vpertama    : boolean;
...
procedure TInfoKartuPiutangFrm.Proc_Refresh2;
var
  vTop        : String;
begin
...
procedure TInfoKartuPiutangFrm.Proc_Refresh3;
var
  i           : word;
  vpertama    : boolean;
...
procedure TInfoKartuPiutangFrm.Proc_Refresh4;
var
  i           : word;
  vpertama    : boolean;
  vFieldName  : array[0..20] of String;
...
procedure TInfoKartuPiutangFrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  InfoKartuPiutangFrm:=Nil;
end;

procedure TInfoKartuPiutangFrm.FormPaint(Sender: TObject);
begin
...
procedure TInfoKartuPiutangFrm.tbRefreshClick(Sender: TObject);
begin
  qB1.Close;
  qB1.Open;
end;

procedure TInfoKartuPiutangFrm.tbExportClick(Sender: TObject);
begin
...
procedure TInfoKartuPiutangFrm.TabSheet1Show(Sender: TObject);
var
  i : integer;
begin
...
procedure TInfoKartuPiutangFrm.wwDBGrid1IButtonClick(Sender: TObject);
var
  vFixedCol : String;
begin
...
procedure TInfoKartuPiutangFrm.dbGrid1UpdateFooter(Sender: TObject);
begin
  if vsql_org1<>'' then
  begin
    qBX.Close;
   ...
   { NON AKTIVKAN DULU KARENA MULTI CURENCY
...
procedure TInfoKartuPiutangFrm.FormCreate(Sender: TObject);
begin
...
}
{
  ...
  case PageControl1.TabIndex of
      0 : tbFilter.Enabled:=True;
      3 : tbFilter.Enabled:=False;
  end;

end;

procedure TInfoKartuPiutangFrm.tbFilterClick(Sender: TObject);
var
  i : Word;
begin
...
procedure TInfoKartuPiutangFrm.dbGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TInfoKartuPiutangFrm.dbGrid1CalcCellColors(Sender: TObject; Field: TField;
...
procedure TInfoKartuPiutangFrm.pTopClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
...
procedure TInfoKartuPiutangFrm.tbPrintClick(Sender: TObject);
begin
...
procedure TInfoKartuPiutangFrm.CheckBox1Click(Sender: TObject);
begin
...
procedure TInfoKartuPiutangFrm.dbGrid2UpdateFooter(Sender: TObject);
begin
...
procedure TInfoKartuPiutangFrm.TabSheet2Show(Sender: TObject);
var
  i : integer;
begin
  qAwal.Open;
...
procedure TInfoKartuPiutangFrm.tbFilter2Click(Sender: TObject);
begin
...
procedure TInfoKartuPiutangFrm.qB1BeforeOpen(DataSet: TDataSet);
begin
  qB1.ParamByName('pmin').AsFloat:=vmin.Value;
  qB1.ParamByName('pmax').AsFloat:=vmax.Value;
end;

procedure TInfoKartuPiutangFrm.qBXBeforeOpen(DataSet: TDataSet);
begin
  qBX.ParamByName('pmin').AsFloat:=vmin.Value;
  qBX.ParamByName('pmax').AsFloat:=vmax.Value;
end;

procedure TInfoKartuPiutangFrm.dbGrid3TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TInfoKartuPiutangFrm.dbGrid3UpdateFooter(Sender: TObject);
begin
  if vsql_org1<>'' then
  begin
    qB3X.Close;
...
procedure TInfoKartuPiutangFrm.TabSheet3Show(Sender: TObject);
var
  i : integer;
begin
...
procedure TInfoKartuPiutangFrm.dbGrid1DblClick(Sender: TObject);
var
  vcatatan : String;
begin
  vcatatan:=qB1CATATAN.AsString;
  if copy(qB1NO_FAKTUR.AsString,3,2)='21' then
  begin
      if InputQuery('CATATAN','Catatan : ',vcatatan) then
      begin
         qCatatan.Close;
         qCatatan.ParamByName('pcatatan').AsString:=vcatatan;
         qCatatan.ParamByName('pno_reg_os').AsFloat:=qB1NO_FAKTUR.AsFloat;
         qCatatan.ExecSQL;
         qB1.Close;
         qB1.Open;
      end;
  ...
  begin
      ...
      begin
         qCatatan2.Close;
         qCatatan2.ParamByName('pcatatan').AsString:=vcatatan;
         qCatatan2.ParamByName('pno_reg_os').AsFloat:=qB1NO_FAKTUR.AsFloat;
         qCatatan2.ExecSQL;
         qB1.Close;
         qB1.Open;
      end;
...
procedure TInfoKartuPiutangFrm.dbGrid4UpdateFooter(Sender: TObject);
begin
 { if vsql_org1<>'' then
  begin
    qB4X.Close;
    qB4X.SQL.Clear;
    qB4X.SQL.Add('select count(*) as ndata, sum(nilai_tagihan) as nilai_tagihan,');
    qB4X.SQL.Add('sum(retur_pengiriman) as  retur_pengiriman, sum(retur_outlet) as  retur_outlet,');
    qB4X.SQL.Add('sum(voucher_klaim) as  voucher_klaim, sum(voucher_lain) as  voucher_lain, sum(koreksi) as  koreksi,');
    qB4X.SQL.Add('sum(bayar_tunai) as  bayar_tunai, sum(bayar_cek) as  bayar_cek, sum(bayar_tt) as  bayar_tt,');
    qB4X.SQL.Add('sum(bayar_pending) as  bayar_pending, sum(sisa_tagihan) as  sisa_tagihan from (');
    qB4X.SQL.Add(vsql_org4+vfilter4+')');                                 //GANTI
    qB4X.Open;
    pLeft2.Caption:=FormatFloat('#,#;0',qB4XNDATA.AsInteger);
    DBGrid4.ColumnByName('NILAI_TAGIHAN').FooterValue:=FormatFloat('#,#;(#,#);-',qB4X.FieldByName('NILAI_TAGIHAN').AsFloat);
    ...
    DBGrid4.ColumnByName('sisa_tagihan').FooterValue:=FormatFloat('#,#;(#,#);-',qB4X.FieldByName('sisa_tagihan').AsFloat);
  end
  else
    pLeft2.Caption:='';  }
...
procedure TInfoKartuPiutangFrm.dbGrid4TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TInfoKartuPiutangFrm.TabSheet4Show(Sender: TObject);
var
  i : integer;
begin
...
procedure TInfoKartuPiutangFrm.BitBtn100Click(Sender: TObject);
var
  i : Integer;
  t1, t2, t3, t4 : REAL;
begin
...
procedure TInfoKartuPiutangFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=EndOfTheMonth(VTglAwal.Date);
end;

procedure TInfoKartuPiutangFrm.qB4CalcFields(DataSet: TDataSet);
begin
  qB4AKHIR.AsFloat:=(qB4AWAL.AsFloat+qB4DEBET.AsFloat)-qB4KREDIT.AsFloat;
end;

procedure TInfoKartuPiutangFrm.CheckBox2Click(Sender: TObject);
begin
qB4.Filtered:=CheckBox2.Checked;
end;

procedure TInfoKartuPiutangFrm.qB4FilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
...
procedure TInfoKartuPiutangFrm.cbBuktiClick(Sender: TObject);
var
  t1, t2 : Real;
  i : Integer;
begin
  ...
  begin
    qB5.Close;
    qb5.ParamByName('pcustomer').AsString:=qB4ID_OUTLET.AsString;
    qb5.ParamByName('pawal').AsDate:=VTglAwal.Date;
    qb5.ParamByName('pakhir').AsDate:=vTglAkhir.Date;
    qB5.Open;
    dbGrid4.SetFocus;
    dbGrid5.Refresh;
    LCustomer.Caption:='Bukti Transaksi '+qB4NAMA_OUTLET.AsString+' - '+qB4MU.AsString;
    pBukti.Visible:=True;
...
procedure TInfoKartuPiutangFrm.dbGrid4KeyPress(Sender: TObject;
  var Key: Char);
begin
  qB5.Close;
  qb5.ParamByName('pcustomer').AsString:=qB4ID_OUTLET.AsString;
...
procedure TInfoKartuPiutangFrm.dbGrid4KeyDown(Sender: TObject;
...
procedure TInfoKartuPiutangFrm.dbGrid4RowChanged(Sender: TObject);
var
 t1, t2 : Real;
 i : Integer;
begin
...
procedure TInfoKartuPiutangFrm.qB4BeforeOpen(DataSet: TDataSet);
begin
...
procedure TInfoKartuPiutangFrm.qB5BeforeOpen(DataSet: TDataSet);
begin
  case rgMU.ItemIndex of
