...
procedure TNeracaFrm.Proc_Refresh2;
var
  i           : word;
  vpertama    : boolean;
  vFieldName  : array[0..50] of String;
...
begin
  ...
  begin
        ...
        if (qB.FieldCount>=1) then                                      //Ganti
        begin
          if vfield_idx=0 then
          begin
            for i:=0 to vNField do
            begin
              if (qB.FieldByName(vFieldName[i]).FieldKind=fkData) and   //Ganti
                 (copy(vFieldName[i],1,3)<>'TGL') then
              begin
                if vpertama then
                  begin
  ...
  if qB.Active then                               //Ganti
    //vorder:=' order by '+vFieldName[0]
    vorder:=' order by NO_IDX'
    else
      vorder:='';
  ...
  qB.DisableControls;                             //Ganti
  qB.Close;                                       //Ganti
  qB.SQL.Text:=vsql_org+vfilter+vorder;           //Ganti
  qB.Open;                                        //Ganti
  dbGrid2.GroupFieldName:=vFieldName[0];           //Ganti
  qB.EnableControls;                              //Ganti
