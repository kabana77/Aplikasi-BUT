...
type
  TBuktiBayarFrm = class(TForm)
    ...
    lcdPerk2: TwwDBLookupComboDlg;
    Label13: TLabel;
    CheckBox1: TCheckBox;
    dbtPerkiraan: TDBText;
    ...
    TabSheet2: TTabSheet;
    qBPerkiraan: TSmartQuery;
    qBPerkiraanKD_PERK: TStringField;
    qBPerkiraanNAMA_PERKIRAAN: TStringField;
    qBPerkiraanNAMA_PERKIRAAN2: TStringField;
    qBPerkiraanISDETAIL: TStringField;
    qBPerkiraanAWAL: TFloatField;
    qBPerkiraanDEBET: TFloatField;
    qBPerkiraanKREDIT: TFloatField;
    qBPerkiraanAKHIR: TFloatField;
    dsqBPerkiraan: TwwDataSource;
    qB2: TSmartQuery;
    ...
    qB2KD_PERK2: TStringField;
    qB2NAMA_PERKIRAAN2: TStringField;
    dsqB2: TwwDataSource;
    pTop2: TPanel;
    Label15: TLabel;
    Label18: TLabel;
    ...
    qB2JNS_BAYAR: TStringField;
    qB2CARA_BAYAR: TStringField;
    qB2x: TOraQuery;
    qB2xNDATA: TFloatField;
    qB2xDEBET: TFloatField;
    qB2xKREDIT: TFloatField;
    qB2Exp: TSmartQuery;
    dsqB2Exp: TwwDataSource;
    qB2ExpNO_REG_OS: TFloatField;
    qB2ExpTGL: TDateTimeField;
    qB2ExpJNS_BAYAR: TStringField;
    qB2ExpCARA_BAYAR: TStringField;
    qB2ExpKETERANGAN: TStringField;
    qB2ExpDEBET: TFloatField;
    qB2ExpKREDIT: TFloatField;
    qB2ExpKD_PERK2: TStringField;
    qB2ExpNAMA_PERKIRAAN2: TStringField;
    qB3: TSmartQuery;
    dsqB3: TwwDataSource;
    TabSheet3: TTabSheet;
    pTop3: TPanel;
    ...
    qB3KD_PERK: TStringField;
    qB3KETERANGAN: TStringField;
    qB3DEBET: TFloatField;
    qB3KREDIT: TFloatField;
    TabSheet4: TTabSheet;
    wwDBGrid1: TwwDBGrid;
    wwIButton4: TwwIButton;
    qKonrolJurnal: TSmartQuery;
    dsqKonrolJurnal: TwwDataSource;
    qKonrolJurnalKD_PERK: TStringField;
    qKonrolJurnalNAMA_PERKIRAAN: TStringField;
    qKonrolJurnalNILAI: TFloatField;
    lcDepo: TwwDBLookupCombo;
    qBDetailKD_DIV: TStringField;
    lcDivisi: TwwDBLookupCombo;
    ...
    procedure lcdPerk2Enter(Sender: TObject);
    ...
    procedure qBPerkiraanBeforeOpen(DataSet: TDataSet);
    procedure dbGrid2TitleButtonClick(Sender: TObject; AFieldName: String);
    procedure qB2ExpBeforeOpen(DataSet: TDataSet);
...
procedure TBuktiBayarFrm.Proc_Refresh2;
var
  i           : word;
  vpertama    : boolean;
...
begin
  ...
  DMFrm.qDivisi.Close;
  DMFrm.qDivisi.Open;
  lcdPerk2.Text:=qPerkiraanKD_PERK.AsString;
...
begin
  ...
  qBMasterKD_PERK2.AsString:='5320';
  qBMasterJNS_BAYAR.AsString:='LL KELUAR';
  qBMasterCARA_BAYAR.AsString:='TUNAI';
  qBMasterKETERANGAN.AsString:='Biaya Operasional';
  qBMasterKD_DEPO.AsString:=DMFrm.qDepoKD_DEPO.AsString;
...
begin
  ...
  if (qBMasterCARA_BAYAR.AsString='CEK/ BG') then qBMasterKD_PERK.AsString:='112';

  if (((qBMasterJNS_BAYAR.AsString='BAYAR AR') or (qBMasterJNS_BAYAR.AsString='BAYAR AP')) and (qBDetail.RecordCount>0) and (qBDetailKD_PERK.AsString<>'') and (qBMasterISPOST.AsString='1')) then qBMasterKD_PERK2.AsString:=qBDetailKD_PERK.AsString;
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          DMFrm.Fno_Bukti.ParamByName('pkode_form').AsString:='119';
...
begin
  ...
  if CheckBox1.Checked then
    qB1.ParamByName('pkd_perk').AsString:='%'
  else
    qB1.ParamByName('pkd_perk').AsString:=qPerkiraanKD_PERK.AsString;
end;
...
begin
  ...
  if CheckBox1.Checked then
    qBX.ParamByName('pkd_perk').AsString:='%'
  else
    qBX.ParamByName('pkd_perk').AsString:=qPerkiraanKD_PERK.AsString;
end;
...
begin
  ...
  qBDetailKD_DIV.AsString:=DMFrm.qDivisiKD_DIV.AsString;
...
procedure TBuktiBayarFrm.qBDetailBeforeInsert(DataSet: TDataSet);
begin
  if (qBMasterISPOST.AsString='1') then
  begin
...
procedure TBuktiBayarFrm.tsInputDJurnalShow(Sender: TObject);
var
  vdebet, vkredit : real;
begin
  qJurnal.Close;
...
procedure TBuktiBayarFrm.lcdPerk2Enter(Sender: TObject);
begin
  qPerkiraan.Open;
end;

procedure TBuktiBayarFrm.CheckBox1Click(Sender: TObject);
begin
  if CheckBox1.Checked then
  begin
    lcdPerk2.Enabled:=False;
    dbtPerkiraan.Enabled:=False;
  end
  else
  begin
    lcdPerk2.Enabled:=True;
    dbtPerkiraan.Enabled:=True;
  end;
end;

procedure TBuktiBayarFrm.TabSheet2Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
...
procedure TBuktiBayarFrm.qB2BeforeOpen(DataSet: TDataSet);
begin
  qB2.ParamByName('pkd_perk').AsString:=qBPerkiraanKD_PERK.AsString;
  qB2.ParamByName('pawal').AsDateTime:=vfield_awal;
  qB2.ParamByName('pakhir').AsDateTime:=vfield_akhir;
end;

procedure TBuktiBayarFrm.qB2xBeforeOpen(DataSet: TDataSet);
begin
  qB2x.ParamByName('pkd_perk').AsString:=qBPerkiraanKD_PERK.AsString;
  qB2x.ParamByName('pawal').AsDateTime:=vfield_awal;
  qB2x.ParamByName('pakhir').AsDateTime:=vfield_akhir;
end;

procedure TBuktiBayarFrm.dbGrid2UpdateFooter(Sender: TObject);
begin
  qB2X.Close;
  qB2X.SQL.Clear;
  qB2X.SQL.Add('select count(*) as ndata, sum(debet) as debet, sum(kredit) as kredit from (');
  qB2X.SQL.Add(vsql_org2+vfilter2+')');                                   //GANTI
  qB2X.Open;
  pLeft2.Caption:=FormatFloat('#,#;0',qB2XNDATA.AsInteger);
  dbGrid2.ColumnByName('DEBET').FooterValue:=FormatFloat('#,#;(#,#);-', qB2XDEBET.AsFloat);
  dbGrid2.ColumnByName('KREDIT').FooterValue:=FormatFloat('#,#;(#,#);-', qB2XKREDIT.AsFloat);

end;

procedure TBuktiBayarFrm.qBPerkiraanBeforeOpen(DataSet: TDataSet);
begin
  qBPerkiraan.ParamByName('pkd_perk').AsString:=lcdPerk2.Text;
  qBPerkiraan.ParamByName('pawal').AsDateTime:=vfield_awal;
  qBPerkiraan.ParamByName('pakhir').AsDateTime:=vfield_akhir;
end;

procedure TBuktiBayarFrm.dbGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
var
  i : integer;
begin
...
procedure TBuktiBayarFrm.qB2ExpBeforeOpen(DataSet: TDataSet);
begin
  qB2Exp.ParamByName('pkd_perk').AsString:=qBPerkiraanKD_PERK.AsString;
  qB2Exp.ParamByName('pawal').AsDateTime:=vfield_awal;
  qB2Exp.ParamByName('pakhir').AsDateTime:=vfield_akhir;
end;

procedure TBuktiBayarFrm.TabSheet3Show(Sender: TObject);
var
  i : integer;
begin
  vkeyfield:='';
  pTop3.Caption:=pTop1.Caption;
  qBPerkiraan.Close;
  qBPerkiraan.Open;
  qB3.Close;
  qB3.Open;
...
procedure TBuktiBayarFrm.qB3BeforeOpen(DataSet: TDataSet);
begin
  qB3.ParamByName('pkd_perk').AsString:=qBPerkiraanKD_PERK.AsString;
  qB3.ParamByName('pawal').AsDateTime:=vfield_awal;
  qB3.ParamByName('pakhir').AsDateTime:=vfield_akhir;
end;

procedure TBuktiBayarFrm.TabSheet4Show(Sender: TObject);
begin
  qKonrolJurnal.Close;
  qKonrolJurnal.Open;
end;

procedure TBuktiBayarFrm.cbJenisCloseUp(Sender: TwwDBComboBox;
  Select: Boolean);
begin
  if Select then
  begin
      if qBMasterJNS_BAYAR.AsString='BAYAR AP' then
      begin
          qBMasterKD_PERK2.AsString:='23';
      end;
  end;
end;
